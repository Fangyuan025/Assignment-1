...
def main():
    ...
    # 2. Define a pipeline with TF-IDF + KNN
    pipeline = Pipeline([
    ...
    cv_csv_filename = "cv_results_knn.csv"
    df_cv.to_csv(cv_csv_filename, index=False, encoding="utf-8")
    print(f"10-fold cross-validation (KNN) completed. The results have been saved to {cv_csv_filename}\n")
    print(df_cv)
    ...
    plt.title("Confusion Matrix (Summed Over 10 Folds, KNN)")
    plt.colorbar()
    ...
    print("A colorful confusion matrix (KNN) has been saved as 'confusion_matrix_knn.png'.")
