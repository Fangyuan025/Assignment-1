...
def main():
    ...
    # 2. Define a pipeline with TF-IDF + Random Forest
    pipeline = Pipeline([
    ...
    cv_csv_filename = "cv_results_rf.csv"
    df_cv.to_csv(cv_csv_filename, index=False, encoding="utf-8")
    print(f"10-fold cross-validation (Random Forest) completed. The results have been saved to {cv_csv_filename}\n")
    print(df_cv)
    ...
    plt.title("Confusion Matrix (Summed Over 10 Folds, Random Forest)")
    plt.colorbar()
    ...
    print("A colorful confusion matrix (Random Forest) has been saved as 'confusion_matrix_rf.png'.")
