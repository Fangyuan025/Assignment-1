...
def main():
    ...
    # 2. Define a pipeline with TF-IDF + Naive Bayes
    pipeline = Pipeline([
    ...
    cv_csv_filename = "cv_results_nb.csv"
    df_cv.to_csv(cv_csv_filename, index=False, encoding="utf-8")
    print(f"10-fold cross-validation (Naive Bayes) completed. The results have been saved to {cv_csv_filename}\n")
    print(df_cv)
    ...
    plt.title("Confusion Matrix (Summed Over 10 Folds, Naive Bayes)")
    plt.colorbar()
    ...
    print("A colorful confusion matrix (Naive Bayes) has been saved as 'confusion_matrix_nb.png'.")
